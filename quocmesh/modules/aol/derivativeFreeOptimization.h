...
//! Nelder-Mead downhill simplex minimization method for continuous functions f : R^n -> R
...
class NelderMeadDownhillSimplexAlgorithm {
  ...
  const int maxIt;
public:
  NelderMeadDownhillSimplexAlgorithm ( aol::Op<aol::Vector<RealType>, aol::Scalar<RealType> > &e, const RealType tol = 1e-6, const int maxIt = 100 ) : _e ( &e ), tol ( tol ), maxIt ( maxIt ) { }
