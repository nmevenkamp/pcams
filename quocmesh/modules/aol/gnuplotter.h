...
class PlotDataFileHandler {
  ...
  std::vector<const char*> _datafileTitleVector;
...
public:
  ...
  void generateIsolineData ( const aol::MultiVector<RealType> &levelsetFunctions,
    ...
    for ( int i = 0; i < levelsetFunctions.numComponents(); i++ )
      generateIsolineData ( levelsetFunctions[i], Grid, ConfDummy, Isovalue, ScaleToUnitDomain, MirrorAtYAxis, Title );
  }
  ...
  void generateBackgroundPNGData ( const PictureType &Image ) {
    ...
    std::string tempFileNameString = tempFileName;
    _datafileFileNameVector.push_back(tempFileNameString);
    _datafilePlotStyleVector.push_back(GNUPLOT_BACKGROUNDPNG);
    _datafileTitleVector.push_back("");
  }
  ...
  void generateFunctionLogPlot ( const aol::Vector<PositionDataType> &FunctionPositions,
    ...
    generateFunctionPlot ( FunctionPositions, logFunctionValues, PlotAsBoxes, Title );
  }
  void generateFunctionPlot ( const std::vector<std::pair<RealType, RealType> > &FunctionPositionsAndValues, const bool PlotAsBoxes = false, const char *Title = "" ) {
    ...
    generateFunctionPlot ( functionPositions, functionValues, PlotAsBoxes, Title );
  }
  void generateFunctionLogPlot ( const std::vector<std::pair<RealType, RealType> > &FunctionPositionsAndValues, const bool PlotAsBoxes = false, const char *Title = "" ) {
    const int numPositions = FunctionPositionsAndValues.size();
  ...
  void generateFunctionPlot ( const aol::Vector<InputDataType> &FunctionValues, const RealType A = 0, const RealType B = 1, const bool PlotAsBoxes = false, const char *Title = "" ) {
    ...
    generateFunctionPlot(positions, FunctionValues, PlotAsBoxes, Title );
  }
  template <typename InputDataType>
  void generateFunctionPlot ( const aol::MultiVector<InputDataType> &FunctionValues, const RealType A = 0, const RealType B = 1, const bool PlotAsBoxes = false ) {
    for ( int i = 0; i < FunctionValues.numComponents(); ++i )
      generateFunctionPlot ( FunctionValues[i], A, B, PlotAsBoxes );
  }
  template <typename InputDataType>
  void generateFunctionLogPlot ( const aol::Vector<InputDataType> &FunctionValues, const RealType A = 0, const RealType B = 1, const bool PlotAsBoxes = false, const char *Title = "" ) {
    aol::Vector<RealType> logFunctionValues;
    toLogScale ( FunctionValues, logFunctionValues );
    generateFunctionPlot(logFunctionValues, A, B, PlotAsBoxes, Title );
  }
  /**
  ...
  void generateCurvePlot ( const aol::RandomAccessContainer<aol::Vec<2, RealType> > &Points, const bool PlotAsPoints = false, const char *Title = "" ) {
    char tempFileName[1024];
  ...
  void generateCurvePlot ( const aol::MultiVector<RealType> &Points, const bool PlotAsPoints = false, const char *Title = "" ) {
    ...
    generateCurvePlot ( pointsRAC, PlotAsPoints, Title );
  }
  
  void generateCurvePlot ( const std::vector<std::pair<RealType, RealType> > &Points, const bool PlotAsPoints = false, const char *Title = "" ) {
    aol::RandomAccessContainer<aol::Vec<2, RealType> > pointsRAC;
  ...
  void generateCurvePlot ( const aol::Vector<RealType> &PointsX, const aol::Vector<RealType> &PointsY, const bool PlotAsPoints = false, const char *Title = "" ) {
  ...
  void generateClosedCurvePlot ( const aol::MultiVector<RealType> &Points, const char *Title = "" ) {
    char tempFileName[1024];
    sprintf ( tempFileName, "curve.datXXXXXX" );
    ofstream tempOutFile;
    generateTemporaryFile ( tempFileName, tempOutFile );
  ...
  const std::vector<const char*> &getDataFileTitles ( ) const {
...
public:
  ...
  void addPlotCommandsFromHandler( const PlotDataFileHandler<RealType> &Handler ){
    ...
    const std::vector<const char*> &datafileTitleVector = Handler.getDataFileTitles();
    for ( unsigned int i = 0; i < datafileFileNameVector.size(); i++ )
      addPlotCommand( datafileFileNameVector[i].c_str(), datafilePlotStyleVector[i], -2, datafileTitleVector[i] );
  }
